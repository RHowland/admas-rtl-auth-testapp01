...
import { PasswordSchema } from "../valibot/SchemaTypes";
...
const PasswordComplexity = ({ password }) => {
  ...
  const checkRule = (rule: string) => {
    if (issues[rule]) return false;
    return true;
  };

  return (
    <div className="flex flex-col gap-1">
      {PasswordSchema.pipe.map((v, index) =>
      {
        if(v.message === undefined) return ('');
        return (
          (
            <p
              key={index}
              className={`flex flex-row items-center text-sm ${
                checkRule(v.expects) ? "text-green-600" : "text-red-600"
              }`}
            >
              {
                checkRule(v.expects) ? (
